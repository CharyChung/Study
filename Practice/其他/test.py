...
W = torch.normal(0, 0.01, size=(batch_size, num_input), requires_grad=True)
...
def cross_entropy(y_pred, y_true):
    return -torch.log(y_pred[range(len(y_pred)), y_true])
...
def accuracy(y_pred, y_true):
    ...
    print(y_pred)
    print(y_true.dtype)
    print(y_pred.type(y_true.dtype))

    cmp = y_pred.type(y_true.dtype) == y
    print(cmp)
